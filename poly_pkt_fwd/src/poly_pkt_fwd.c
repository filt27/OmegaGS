...
int main(void)
{
	...
	if (radiostream_enabled == true) {
		MSG("INFO: [main] Starting the concentrator %s\n", SPI_DEV_PATH);
		i = lgw_start();
		
		if (i == LGW_HAL_SUCCESS) {
			MSG("INFO: [main] concentrator %s started, radio packets can now be received.\n", SPI_DEV_PATH);
		} else {
	...
	//printf(downstream_enabled ? "true" : "false"); //-> true
	if (downstream_enabled == true) {
	...
	while (!exit_sig && !quit_sig) {
		...
		while(concentrator_chosen() == false){			
		}
...
void thread_up(void) {
	...
	*(uint32_t *)(buff_up + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		//ovo se izvrsava stalno u pozadini
		...
		for (i=0; i < nb_pkt; ++i) {
			...
			/*db_value = 0;
			int rc = sqlite3_open ("/opt/ttn-gateway/packet_forwarder/poly_pkt_fwd/src/sensordata2.db", &db);
			...
			snprintf(sql, sizeof(sql), "SELECT (%.2f - received_at) FROM data3 WHERE device_eui = '%s' AND direction = 'up' order by id desc limit 1;", (long double)time(0), "DEVEUI");
			//AND payload LIKE '%#012x' AND sent_on_server = 1
			printf(sql);
			
			rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
			
			//db_value = sqlite3_column_int(stmt, 0);
			...
			if(db_value < 3){
				printf("\n######## - - NOT INSERTING - SAME DEVICE IN LAST 3s !! - - ########\n");
				flag_already_sent = true;
			}
			else{
				printf("\n######## - - NEW DATA INSERTED!! - - ########\n");
				flag_already_sent = false;
			}
			...
			if(flag_already_sent){
				snprintf(sql, sizeof(sql), "INSERT INTO data3 (gateway_eui, concentrator, device_eui, payload, size, modulation, bw, sf, rssi, snr, internal_counter, current_date, received_at, sent_on_server, direction) VALUES ('B827EBFFFECDC406', '%s', 'DEVEUI', '%#012x', '%d', '%d', '%d', '%d', '%.2f', '%.2f', '%d', '%s', '%.2f', '%d', 'up');", SPI_DEV_PATH, p->payload, p->size, p->modulation, p->bandwidth, p->datarate, p->rssi, p->snr, p->count_us, fetch_timestamp, (long double)time(0), 0);
			}
			else {
				snprintf(sql, sizeof(sql), "INSERT INTO data3 (gateway_eui, concentrator, device_eui, payload, size, modulation, bw, sf, rssi, snr, internal_counter, current_date, received_at, sent_on_server, direction) VALUES ('B827EBFFFECDC406', '%s', 'DEVEUI', '%#012x', '%d', '%d', '%d', '%d', '%.2f', '%.2f', '%d', '%s', '%.2f', '%d', 'up');", SPI_DEV_PATH, p->payload, p->size, p->modulation, p->bandwidth, p->datarate, p->rssi, p->snr, p->count_us, fetch_timestamp, (long double)time(0), 1);
			}
			...
			flag_already_sent = false;  		//for debugging			
		...
		/*printf("############### - %s - ## RECEIVING RECEIVING ###############\n", SPI_DEV_PATH);
			
		for (i=0; i<18; i++){
			printf("%02X | ", p->payload[2*i]);
		}*/
...
void thread_down(void* pic) {
	...
	*(uint32_t *)(buff_req + 8) = net_mac_l;
	...
	bool flag_send_with_this_concentrator = true;
...
///////////////////////////////////////////////////
			...
			// treba odrediti device EUI prije ovoga -> za sada je rucno postavljeno na "DEVEUI"
						
			
			snprintf(sql, sizeof(sql), "select concentrator from data3 where snr = (select max(snr) from data3 where device_eui = 'DEVEUI' and direction = 'up' order by id desc limit 30);");
			//AND payload LIKE '%#012x' AND sent_on_server = 1
			printf(sql);
			...
			printf("\nConcentrator with best RSSI = %s\n", db_value);
			
			
			if(!strcmp(SPI_DEV_PATH, db_value)){
				printf("\n######## - - SENDING WITH CONCENTRATOR %s - - ########\n", db_value);
				printf("SPI_DEV_PATH = %s == %s = db_value", SPI_DEV_PATH, db_value);
				flag_send_with_this_concentrator = true;
			}
			else{
				printf("\n######## - - WAITING FOR THE RIGHT CONCNETRATOR TO SEND - - ########\n");
				printf("SPI_DEV_PATH = %s != %s = db_value", SPI_DEV_PATH, db_value);
				flag_send_with_this_concentrator = false;
			}
			...
			if(flag_send_with_this_concentrator) {
		
				snprintf(sql, sizeof(sql), "INSERT INTO data3 (gateway_eui, concentrator, device_eui, payload, size, modulation, bw, sf, received_at, sent_on_server, direction) VALUES ('B827EBFFFECDC406', '%s', 'DEVEUI', '%s', '%d', '%d', '%d', '%d',  '%.2f', '%d', 'down');", SPI_DEV_PATH, txpkt.payload, txpkt.size, txpkt.modulation, txpkt.bandwidth, txpkt.datarate, (long double)time(0), 0);
			...
			flag_send_with_this_concentrator = true;	//for debugging
			
			
			if(flag_send_with_this_concentrator){
				
				/*printf("############### - %s - ## SENDING SENDING ###############\n", SPI_DEV_PATH);
...
//////////////////////////////////////////////////////////////////////////////
	...
	MSG("######################packet_forwarder/poly_pkt_fwd/src/poly_pkt_fwd.c: (thread_down) Downstream Packet RF power = %d dBm\n", rf_power_level);
...
bool concentrator_chosen(){
	time_t now;
	now = time(NULL);		
		
	if((strcmp(SPI_DEV_PATH, "/dev/spidev0.0")==0) || (strcmp(SPI_DEV_PATH, "/dev/spidev1.1")==0)){
		if(now % 6 < 3){
			return true;
		} else{
			return false;
	...
	if((strcmp(SPI_DEV_PATH, "/dev/spidev0.1")==0) || (strcmp(SPI_DEV_PATH, "/dev/spidev1.2")==0)){
		if(now % 6 >= 3){
			return true;
		} else{
			return false;
